...
@section('content_header')
    <h1>Consultar o acervo</h1>
    <ol class="breadcrumb">
        <li><a href="{{ route('home') }}"><i class="fas fa-home"></i></a></li>
        <li><a href="{{ route('search') }}">Consultar o acervo</a></li>
    </ol>
@stop
        ...
        <div class="col-md-12">
            <table id="tb_filmes" class="table table-striped">
                ...
                <tbody>
                    ...
                    <tr>
                        <td>{{$i->movie->title}}</td>
                        <td>{{$i->movie->original_title}}</td>
                        <td>{{$i->movie->available_genres}}</td>
                        <td>{{$i->movie->year}}</td>
                        <td>{{$i->movie->available_type}}</td>
                        <td>{{$i->media->description}}</td>
                        <td> 
                            @if($i->status == 'Disponível')
                                <span class="badge bg-green">{{$i->status}}</span> 
                            @else
                                <span class="badge bg-red">{{$i->status}}</span> 
                            @endif
                        </td>
                        <td>  
                            <div class="btn-group">
                                    <abbr title="Detalhes"><a chave="{{ route('movie_details', $i->movie->id) }}" class="btn btn-default btn-sm btn-exibir" style="color:black"><i class="fas fa-list-alt"></i> &nbsp; &nbsp; Detalhes</a></abbr>
                            </div>
                        </td>
                    </tr>
                    @endforeach 
